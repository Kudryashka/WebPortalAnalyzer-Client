export const SERVER_URL = 'http://xxx/';

export const REST_API_URL = `${SERVER_URL}api/`;

export const LOGIN_URL = `${REST_API_URL}login`;

export const REST_LINKS_CHECK_RUN_URL =     	`${REST_API_URL}v1.0/portalLinksCheck/run/`;
export const REST_LINKS_CHECK_STOP_URL = 		`${REST_API_URL}v1.0/portalLinksCheck/stop/`;
export const REST_LINKS_CHECK_STATUS_URL = 		`${REST_API_URL}v1.0/portalLinksCheck/status/`;
export const REST_LINKS_CHECK_GET_REPORT_URL = 	`${REST_API_URL}/v1.1/linksCheck/report/`;
export const REST_LINKS_CHECK_GET_DETAILS_URL = `${REST_API_URL}/v1.1/linksCheck/details/`;

export const REST_SEARCH_URL = `${REST_API_URL}v1.0/portalEmptySearch/report/`;

export const REST_USER_INFO_URL = `${REST_API_URL}v1.0/portalUsersInfo/report/`;

export const SETTINGS_URL = `${REST_API_URL}v1.0/portalSettings`;